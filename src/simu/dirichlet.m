...
for j=1:nestados %j = cada estado trafegavel
	[row, colum, estados_aux] = find([Mapa_A(Ay(j)-1, Ax(j)) Mapa_A(Ay(j)+1, Ax(j)) Mapa_A(Ay(j), Ax(j)-1) Mapa_A(Ay(j), Ax(j)+1)])
...
plot(Ox, Oy, 's', 'markersize', 6, 'markerfacecolor', 'k', 'markeredgecolor', 'k' );
%salvar resultado do planejamento
theta_des  = atan2(dyn, dxn);
save plano theta_des vxr vyr dxn dyn
pause(0.5)